...
/// Books list screen with filter chips, "Load More" button, and pull-to-refresh.
...
class _BooksScreenState extends State<BooksScreen> {
  ...
  Widget _buildContent(BooksProvider provider) {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount: provider.books.length + 1, // +1 for Load More or end indicator
        itemBuilder: (ctx, i) {
          // Last item: Load More button or "All loaded" indicator
          if (i == provider.books.length) {
            return _buildLoadMoreOrEnd(provider);
          }
  ...
  Widget _buildLoadMoreOrEnd(BooksProvider provider) {
    ...
    if (!provider.hasMore) {
      return Padding(
        ...
        child: Center(
          child: Text(
            'All ${provider.books.length} books loaded',
            key: const Key('books_all_loaded'),
    ...
    return Padding(
      ...
      child: Center(
        child: FilledButton.tonalIcon(
          key: const Key('books_btn_load_more'),
          onPressed: () => provider.loadMore(),
          icon: const Icon(Icons.expand_more),
          label: Text('Load More (${provider.books.length} / ${provider.pagination?.totalItems ?? "??"})'),
        ),
      ),
    );
  }
          ...
          child: Row(
            children: [
              ...
              Container(
                ...
                child: Icon(Icons.menu_book_rounded, color: coverColor, size: 28),
              ),
